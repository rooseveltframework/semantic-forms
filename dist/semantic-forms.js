...
document.addEventListener("keydown",e=>{if(keyCommands.some(command=>command.key.toLowerCase()===e.key.toLowerCase())){const command=keyCommands.find(command=>{const matchesKey=command.key.toLowerCase()===e.key.toLowerCase();let matchesModifier;if(command.modifier){if(command.modifier==="alt")matchesModifier=e.altKey;else if(command.modifier==="ctrl"||command.modifier==="meta")matchesModifier=e.ctrlKey||e.metaKey}else matchesModifier=e.ctrlKey||e.metaKey;return matchesKey&&matchesModifier});if(command&&command.modifier==="alt"&&e.altKey||e.ctrlKey||e.metaKey){e.preventDefault();command.input.focus()}}});
...
if(input.getAttribute("data-focus-key")!==null){const key=input.getAttribute("data-focus-key");const modifier=input.getAttribute("data-focus-modifier");keyCommands.push({key,modifier,input});const indicator=document.createElement("span");indicator.classList.add("focus-key");let spanText="";if(modifier==="alt")
// keyCodes.altKey = true
spanText+=navigator.userAgent.indexOf("Mac")!=-1?"⌥":"alt";else
// keyCodes.ctrlKey = true
// keyCodes.metaKey = true
spanText+=navigator.userAgent.indexOf("Mac")!=-1?"⌘":"ctrl";spanText+=` ${key}`;indicator.innerText=spanText;insertAfter(indicator,newLabel)}
// #endregion
