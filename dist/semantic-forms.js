...
// get focus modifier
let modifier=input.getAttribute("data-focus-modifier");if(modifier)modifier=modifier.toLowerCase();const validModifiers=[null,"alt","ctrl","control","cmd","command","meta"];if(!validModifiers.includes(modifier)){console.error(`Provided modifier key "${modifier}" is not valid. Defaulting to control/command.`);modifier="meta"}keyCommands.push({key,modifier,input});
// create focus indicator for input
const indicator=document.createElement("span");indicator.classList.add("focus-key");let html="";if(modifier==="alt")html+=`<kbd>${navigator.userAgent.indexOf("Mac")!=-1?"⌥":"alt"}</kbd>`;else html+=`<kbd>${navigator.userAgent.indexOf("Mac")!=-1?"⌘":"ctrl"}</kbd>`;html+=` ${key.toUpperCase()}`;indicator.innerHTML=html;insertAfter(indicator,newLabel)}
// #endregion
