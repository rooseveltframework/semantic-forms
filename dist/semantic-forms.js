...
if(!window.semanticFormsObserver){window.semanticFormsObserver=new window.MutationObserver((mutations=>{let stop=false;for(const mutation of mutations){for(const node of mutation.addedNodes)if(node.nodeName==="FORM"){window.semanticForms();stop=true}if(stop)break}}));window.semanticFormsObserver.observe(document.body,{attributes:false,childList:true,characterData:false,subtree:true})}window.semanticForms.reinitialize=form=>{form.classList.remove("semanticFormsActive");window.semanticForms()}}
/***/
/******/}};
/************************************************************************/
/******/ // The module cache
